...
export class RoomController {
    ...
    async list(@Req() req: IRequest, @Res() res: Response,
        ...
        return response(res, false, rooms, { row_count: rooms.length, total_count: availableRooms.length }, null, StatusCodesEnum.SUCCESS);
    }
    ...
    async addMember(@Req() req: IRequest, @Res() res: Response, @Param('room_id') roomId: string, @Body() data: RoomMemberRequestModel): Promise<Response<IResponse>> {
        ...
        await roomMemberService.delete({ member_room_id: roomId, member_user_id: req.credentials.user_id, delete_forever: true });
        await roomMemberService.create(roomId, req.credentials.user_id, UserRolesEnum.ADMIN);

        for (const member of data.members) {
            if (req.credentials.user_id == member.id) continue;
            await roomMemberService.delete({ member_room_id: roomId, member_user_id: member.id, delete_forever: true });
    ...
    async unsubscribe(@Req() req: IRequest, @Res() res: Response, @Param('room_id') roomId: string): Promise<Response<IResponse>> {
        ...
        await roomMemberService.delete({member_room_id: roomId, member_user_id: req.credentials.user_id, delete_forever: false });
