...
export class RoomMemberService implements IService {
    ...
    public async delete({member_room_id, member_user_id, delete_forever}: { member_room_id?: string, member_user_id?: string, delete_forever: boolean }): Promise<boolean> {
        if (delete_forever) {
            return !!(await this.roomMemberRepo.delete({member_room_id, member_user_id}));
        }
